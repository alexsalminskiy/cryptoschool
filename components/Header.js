  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-purple-900/20 bg-background/80 backdrop-blur-xl">
      <div className="container mx-auto px-3 sm:px-4">
        ...
        {/* Mobile Nav */}
        {mobileMenuOpen && (
          <nav className="sm:hidden py-4 space-y-2 border-t border-purple-900/20 animate-in slide-in-from-top-2">
            {/* Nav links */}
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="block py-3 px-4 rounded-lg hover:bg-purple-500/20 text-lg"
            ...
            {user ? (
              <>
                <div className="py-3 px-4 text-sm text-slate-400 border-t border-purple-900/20 mt-2">
                  {profile?.first_name || user.email}
                </div>
                {isAdmin && (
                  <Link
                    href="/admin"
                    className="flex items-center py-3 px-4 rounded-lg hover:bg-purple-500/20 text-lg"
                    onClick={() => setMobileMenuOpen(false)}
                  >
                ...
                <button
                  onClick={handleSignOut}
                  disabled={signingOut}
                  className="w-full flex items-center py-3 px-4 rounded-lg hover:bg-red-500/20 text-red-400 text-lg"
                >
                  {signingOut ? (
                    <><Loader2 className="mr-3 h-5 w-5 animate-spin" /> Выход...</>
                  ) : (
              ...
              <div className="flex flex-col gap-2 pt-2 border-t border-purple-900/20">
                <Link
                  href="/sign-in"
                  className="py-3 px-4 rounded-lg hover:bg-purple-500/20 text-center text-lg"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {t.signIn}
                </Link>
                <Link
                  href="/sign-up"
                  className="py-3 px-4 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 text-white text-center text-lg font-medium mx-4"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {t.signUp}
                </Link>
